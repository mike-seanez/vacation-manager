...
const HolidayList = ({ holidays, selectedYear, onEditHoliday, onDeleteHoliday }) => {
  ...
  const groupedHolidays = sortedHolidays?.reduce((groups, holiday) => {
    const month = new Date(holiday.date)?.toLocaleDateString('es-MX', { month: 'long' });
    if (!groups?.[month]) {
      groups[month] = [];
    }
      ...
      <div className="p-4 max-h-[600px] overflow-y-auto">
        {Object.keys(groupedHolidays)?.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-muted-foreground">No hay días festivos definidos para {selectedYear}</p>
            <p className="text-sm text-muted-foreground mt-2">
              Haz clic en cualquier fecha del calendario para agregar un día festivo
            </p>
          </div>
        ) : (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center space-x-2 mb-1">
                            <Icon
                              ...
                              size={14}
                              className={
                                holiday?.type === 'national' ? 'text-red-600' : 'text-blue-600'
                              }
                            />
